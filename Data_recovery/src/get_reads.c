...
int main(int argc, char *argv[]) {
    if (argc != 5) {
        fprintf(stderr, "Usage: %s <m> <cov> <input_name> <output_name>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    sscanf(argv[1], "%d", &m);
    sscanf(argv[2], "%d", &cov);
	sscanf(argv[3],"%s",input_name);
	sscanf(argv[4],"%s",output_name);

    // Open the query file (input FASTQ file)
    FILE *query_file = fopen(input_name, "rt");
    if (query_file == NULL) {
    ...
    FILE *output_file = fopen(output_name, "wt");
    if (output_file == NULL) {
