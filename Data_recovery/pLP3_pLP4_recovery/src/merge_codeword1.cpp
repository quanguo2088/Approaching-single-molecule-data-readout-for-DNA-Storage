...
int main(int argc, char *argv[]) {
    ...
    // char errname[500] = {0};
    // sprintf(errname, "../results/current_error1.txt");
    // fcurrent_error = fopen(errname, "a");
    // if (fcurrent_error == NULL)
    //     printf("Can not open %s\n", errname);

    // char ref_codeword[70000] = {0};
    // FILE *fref = fopen("../reference/original_codeword1.txt", "rt");
    // if (fref == NULL) {
    ...
    sprintf(counts_file, "../results/487_count_%d.txt", fqindex-1);
    sprintf(counts_file2, "../results/487_count_%d.txt", fqindex);
    ...
    printf("Current erasure rate (pLP3): %lf\n", current_erasure_rate);
